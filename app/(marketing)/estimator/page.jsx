...
function calc(inp) {
  ...
  const bd = [0, 1, 2, 3, 4].map(idx => {
    ...
    return { idx, icon, si, sh, amt: Math.round(ann * sh * b) };
...
function calcPeer(inp, ent, PM_L, PL_L) {
...
// ═════════════════ SUB-COMPONENTS ═════════════════
function Anim({ value, suffix, formatCurrency: fc }) {
...
function generateBoardReport(inp, ent, peer, IL_L, formatCurrency) {
    ...
    <div style={{ marginTop: 20, padding: "14px 16px", borderRadius: 10, border: `1px solid ${C.bdr}`, background: "rgba(11,14,24,0.4)", display: "flex", justifyContent: "center", gap: 20, flexWrap: "wrap", alignItems: "center" }}>
      {[
        ...
        <div key={item.label} style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <div>
            <div style={{ fontSize: 10, fontWeight: 600, color: C.t1, letterSpacing: ".02em" }}>{item.label}</div>
            <div style={{ fontSize: 8, color: C.t3 }}>{item.sub}</div>
          </div>
        </div>
      ))}
    </div>
...
export default function Cockpit() {
  var { t, formatCurrency, locale } = useI18n();
  ...
  const run = useCallback(() => {
    const r = calc(inp);
    // Attach i18n labels to breakdown items
      ...
      <div style={{ maxWidth: 660, margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 18 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
            ...
            <span style={{ fontSize: 8, color: C.t3, fontFamily: MO, padding: "2px 5px", borderRadius: 3, border: `1px solid ${C.bdr}` }}>{t("est.version")}</span>
          </div>
          <div style={{ display: "flex", gap: 2, padding: 3, borderRadius: 8, background: "rgba(11,14,24,.5)", border: `1px solid ${C.bdr}` }}>
            {[["entropy", t("est.tab.ghosttax")], ["peergap", t("est.tab.peergap")], ["report", t("est.tab.report")]].map(([k, l]) => {
          ...
          <>
            {step < 3 && (
              <div style={{ textAlign: "center", marginBottom: 22 }}>
                <p style={{ fontSize: 9, fontWeight: 600, letterSpacing: ".16em", textTransform: "uppercase", color: C.accent, fontFamily: MO, marginBottom: 8 }}>{t("est.hero.badge")}</p>
                <h1 style={{ fontSize: 24, fontWeight: 800, lineHeight: 1.15, letterSpacing: "-.02em", marginBottom: 6 }}>
                  ...
                  <span style={{ color: C.teal }}>{t("est.hero.title2")}</span>
                </h1>
            ...
            {step === 0 && (
              <div style={{ ...gl, padding: 20 }}>
                <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                  <div>
                    <label style={{ display: "block", fontSize: 9, letterSpacing: ".08em", textTransform: "uppercase", color: C.t3, fontWeight: 500, marginBottom: 5 }}>{t("est.label.headcount")}</label>
                  ...
                  <div>
                    <label style={{ display: "block", fontSize: 9, letterSpacing: ".08em", textTransform: "uppercase", color: C.t3, fontWeight: 500, marginBottom: 5 }}>{t("est.label.industry")}</label>
                  ...
                  <div>
                    <label style={{ display: "block", fontSize: 9, letterSpacing: ".08em", textTransform: "uppercase", color: C.t3, fontWeight: 500, marginBottom: 5 }}>{t("est.label.tools")}</label>
                    <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            ...
            {step === 1 && (
              <div style={{ ...gl, padding: 20 }}>
                <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                  {[["saas", t("est.label.saas")], ["cloud", t("est.label.cloud")], ["ai", t("est.label.ai")]].map(([k, l]) => (
                    <div key={k}>
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "9px 12px", borderRadius: 7, background: "rgba(59,130,246,.04)", border: "1px solid rgba(59,130,246,.12)" }}>
                    <span style={{ fontSize: 9, letterSpacing: ".06em", textTransform: "uppercase", color: C.t3 }}>{t("est.label.totalit")}</span>
                    <span style={{ fontFamily: MO, fontSize: 17, fontWeight: 700, color: C.ahi }}>{formatCurrency(tot)}</span>
                  </div>
                  <div style={{ display: "flex", gap: 10 }}>
                    ...
                    <button onClick={() => setStep(2)} style={{ flex: 1, padding: "13px", borderRadius: 8, border: "none", background: C.accent, color: "#fff", fontSize: 12, fontWeight: 700, letterSpacing: ".05em", textTransform: "uppercase", cursor: "pointer" }}>{t("est.btn.next")} →</button>
                  </div>
                </div>
              </div>
            )}
            ...
            {step === 2 && (
              <div style={{ ...gl, padding: 20 }}>
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  <p style={{ fontSize: 12, color: C.t2, lineHeight: 1.5, marginBottom: 2 }}>{t("est.signals.intro")}</p>
                  {SG.map((l, i) => (
            ...
            {step === 3 && res && (
              <div ref={rRef} style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                <div style={{ textAlign: "center", ...rv(1) }}>
                  <p style={{ fontSize: 9, letterSpacing: ".12em", textTransform: "uppercase", fontWeight: 600, color: res.grav >= 61 ? C.red : res.grav >= 31 ? C.ora : C.grn, marginBottom: 5 }}>{t("est.result.detected")}</p>
                  ...
                  <p style={{ fontSize: 11, color: C.t3, marginTop: 3 }}>{t("est.result.range")}: {formatCurrency(res.lo)} – {formatCurrency(res.hi)}</p>
                </div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, ...rv(1, 160) }}>
                  ...
                  <div style={{ padding: 10, borderRadius: 9, background: "rgba(11,14,24,.5)", border: `1px solid ${C.bdr}`, textAlign: "center" }}>
                    <p style={{ fontSize: 7.5, color: C.t3, textTransform: "uppercase", letterSpacing: ".08em", marginBottom: 4 }}>{t("est.result.coefficient")}</p>
                    <p style={{ fontFamily: MO, fontSize: 20, fontWeight: 700, color: res.kappa > .2 ? C.ora : C.ahi }}>{"\u03BA"} = {res.kappa.toFixed(3)}</p>
                    <p style={{ fontSize: 8, color: C.t3, marginTop: 2 }}>{res.kappa > .2 ? t("est.result.kappa.high") : res.kappa > .1 ? t("est.result.kappa.mod") : t("est.result.kappa.low")}</p>
                  </div>
                  <div style={{ padding: 10, borderRadius: 9, background: "rgba(11,14,24,.5)", border: `1px solid ${C.bdr}`, textAlign: "center" }}>
                    <p style={{ fontSize: 7.5, color: C.t3, textTransform: "uppercase", letterSpacing: ".08em", marginBottom: 4 }}>{t("est.result.roi")}</p>
                    <p style={{ fontFamily: MO, fontSize: 20, fontWeight: 700, color: C.teal }}>×{res.roi}</p>
                    <p style={{ fontSize: 8, color: C.t3, marginTop: 2 }}>$990 → {formatCurrency(res.mid * .6, true)} {t("est.result.recovered")}</p>
                  </div>
                </div>

                <div style={{ ...gl, padding: 14, ...rv(2) }}>
                  <h3 style={{ fontSize: 8, letterSpacing: ".1em", textTransform: "uppercase", fontWeight: 600, color: C.t3, marginBottom: 10 }}>{t("est.result.breakdown")}</h3>
                        ...
                        <div key={it.label} style={{ display: "flex", alignItems: "center", gap: 9 }}>
                          ...
                          <div style={{ flex: 1 }}>
                            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 3 }}>
                              ...
                              <span style={{ fontFamily: MO, fontSize: 11, fontWeight: 600, color: C.t1 }}>{formatCurrency(it.amt, true)}/yr</span>
                            </div>
                  ...
                  <div style={{ marginTop: 10, padding: "7px 10px", borderRadius: 5, background: "rgba(59,130,246,.04)", border: "1px solid rgba(59,130,246,.1)", display: "flex", justifyContent: "space-between" }}>
                    <span style={{ fontSize: 8.5, letterSpacing: ".06em", textTransform: "uppercase", color: C.t3 }}>{t("est.result.shadowai")}</span>
                    <span style={{ fontFamily: MO, fontSize: 11, fontWeight: 700, color: C.ora }}>{formatCurrency(res.redund, true)}/yr</span>
                  </div>
                </div>

                <div style={{ ...gl, padding: 14, ...rv(3) }}>
                  <h3 style={{ fontSize: 8, letterSpacing: ".1em", textTransform: "uppercase", fontWeight: 600, color: C.t3, marginBottom: 10 }}>{t("est.result.trajectory")}</h3>
                  ...
                  <div style={{ display: "flex", justifyContent: "center", gap: 14, marginTop: 7, fontSize: 7.5, color: C.t3, textTransform: "uppercase", letterSpacing: ".05em" }}>
                    <span style={{ display: "flex", alignItems: "center", gap: 4 }}><span style={{ width: 8, height: 8, borderRadius: 2, background: C.ora }} />{t("est.result.ungoverned")}</span>
                    <span style={{ display: "flex", alignItems: "center", gap: 4 }}><span style={{ width: 8, height: 8, borderRadius: 2, background: C.teal }} />{t("est.result.governed")}</span>
                  </div>
                  {res.sav24 > 0 && (
                    <div style={{ marginTop: 10, padding: "7px 10px", borderRadius: 5, background: "rgba(52,211,153,.04)", border: "1px solid rgba(52,211,153,.1)", textAlign: "center" }}>
                      <span style={{ fontSize: 10, color: C.t2 }}>{t("est.result.savings24")}: </span>
                      <span style={{ fontFamily: MO, fontSize: 14, fontWeight: 700, color: C.teal }}>{formatCurrency(res.sav24, true)}</span>
                    </div>
                  )}
                </div>
                ...
                <div style={{ ...gl, padding: 20, textAlign: "center", ...rv(3, 160) }}>
                  {cap ? (
                    <div>
                      <p style={{ fontSize: 15, fontWeight: 700, color: C.teal, marginBottom: 5 }}>{"\u2713"} {t("est.cta.registered")}</p>
                      <p style={{ fontSize: 11, color: C.t2 }}>{t("est.cta.checkinbox")}</p>
                    </div>
                  ) : (
                    <>
                      ...
                      <p style={{ fontSize: 7.5, color: C.t3, marginTop: 7 }}>{t("est.cta.stealth")}</p>
                    </>
                  )}
                </div>
                ...
                <div style={{ display: "flex", justifyContent: "center", gap: 8, flexWrap: "wrap" }}>
                  ...
                  <button onClick={() => { setStep(0); setRes(null); setPh(0); setCap(false); setEmail(""); }} style={{ padding: "8px 14px", borderRadius: 7, border: `1px solid ${C.bdr}`, background: "transparent", color: C.t3, fontSize: 10, cursor: "pointer" }}>↻ {t("est.btn.reset")}</button>
                </div>
              </div>
            )}
          </>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            <div style={{ ...gl, padding: 16, textAlign: "center" }}>
              ...
              <p style={{ fontSize: 13, color: C.t2, lineHeight: 1.5, maxWidth: 460, margin: "0 auto 10px" }}>
                {t("est.peer.efficiency")} <strong style={{ color: peer.ov > 75 ? C.red : peer.ov > 50 ? C.ora : C.grn, fontWeight: 700 }}>{t("est.peer.lessthan")} {peer.ov}%</strong> {t("est.peer.ofcomp")} {peer.peer}.
              </p>
              <div style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "6px 14px", borderRadius: 7, background: "rgba(0,0,0,.18)", border: `1px solid ${(peer.ov > 50 ? C.red : C.accent) + "22"}` }}>
                <span style={{ fontSize: 8, color: C.t3, textTransform: "uppercase", letterSpacing: ".08em" }}>{t("est.peer.overall")}</span>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "230px 1fr", gap: 10 }}>
              <div style={{ ...gl, padding: 12, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
                <p style={{ fontSize: 8, fontFamily: MO, color: C.t3, letterSpacing: ".1em", textTransform: "uppercase", marginBottom: 4 }}>{t("est.peer.radar")}</p>
                <Radar mets={peer.mets} />
              </div>
              <div style={{ ...gl, padding: 10, display: "flex", flexDirection: "column", gap: 6 }}>
                <p style={{ fontSize: 8, fontFamily: MO, color: C.t3, letterSpacing: ".1em", textTransform: "uppercase" }}>{t("est.peer.metrics")}</p>
                {peer.mets.map(m => {
                  ...
                  return (
                    <div key={m.k} style={{ padding: "8px 10px", borderRadius: 6, background: bg, border: `1px solid ${col}12` }}>
                      ...
                      <div style={{ display: "flex", justifyContent: "space-between", marginTop: 2 }}>
                        <span style={{ fontSize: 6.5, fontFamily: MO, color: C.t3 }}>{t("est.peer.top10")}</span>
                        <span style={{ fontSize: 6.5, fontFamily: MO, color: col, fontWeight: 600 }}>P{m.pct}</span>
                        <span style={{ fontSize: 6.5, fontFamily: MO, color: C.t3 }}>{t("est.peer.btm10")}</span>
                      </div>
                    </div>
                  );
                })}
            ...
            <div style={{ ...gl, padding: 20, textAlign: "center" }}>
              <p style={{ fontSize: 8, fontFamily: MO, fontWeight: 600, letterSpacing: ".12em", textTransform: "uppercase", color: C.red, marginBottom: 4 }}>{t("est.peer.reclaim")}</p>
              <p style={{ fontFamily: MO, fontSize: 32, fontWeight: 800, color: C.teal, lineHeight: 1, letterSpacing: "-.02em", marginBottom: 3 }}>{formatCurrency(peer.ghost)}<span style={{ fontSize: 13, color: C.t3, fontWeight: 400 }}>/yr</span></p>
              <p style={{ fontSize: 9, color: C.t3, marginBottom: 14 }}>{t("est.peer.recovery60")}</p>
              <div style={{ display: "flex", justifyContent: "center", gap: 8 }}>
                <button onClick={() => setTab("entropy")} style={{ padding: "9px 16px", borderRadius: 7, border: `1px solid ${C.bdr}`, background: "transparent", color: C.t2, fontSize: 10, cursor: "pointer" }}>← {t("est.peer.diagnostic")}</button>
                <button onClick={() => setTab("report")} style={{ padding: "9px 16px", borderRadius: 7, border: `1px solid ${C.teal}25`, background: "rgba(52,211,153,.06)", color: C.teal, fontSize: 10, fontWeight: 600, cursor: "pointer" }}>{t("est.btn.boardreport")} ↓</button>
        ...
        {tab === "report" && res && (
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            <div style={{ ...gl, padding: 20, textAlign: "center" }}>
              <p style={{ fontSize: 9, fontFamily: MO, fontWeight: 600, letterSpacing: ".14em", textTransform: "uppercase", color: C.accent, marginBottom: 8 }}>{t("est.report.label")}</p>
            ...
            <div style={{ display: "flex", justifyContent: "center", gap: 8 }}>
              <button onClick={() => setTab("entropy")} style={{ padding: "9px 16px", borderRadius: 7, border: `1px solid ${C.bdr}`, background: "transparent", color: C.t2, fontSize: 10, cursor: "pointer" }}>← {t("est.peer.diagnostic")}</button>
              <button onClick={() => downloadReport(generateBoardReport(inp, res, peer, IL, formatCurrency))} style={{ padding: "10px 22px", borderRadius: 7, border: "none", background: C.accent, color: "#fff", fontSize: 11, fontWeight: 700, letterSpacing: ".04em", textTransform: "uppercase", cursor: "pointer" }}>
